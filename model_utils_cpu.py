...
class FaceAntiSpoofModel:
    """Class to handle model operations for face anti-spoofing detection (CPU optimized)"""
    ...
    def load_model(self):
        """Load the trained model from checkpoint with CPU optimization"""
        ...
        try:
            ...
            with tf.device('/CPU:0'):
                ...
                # Create an optimized CPU-specific model
                input_shape = (self.img_width, self.img_height, 3)
                dummy_input = np.zeros((1, *input_shape), dtype=np.float32)
                
                # Run a warmup prediction to ensure everything is loaded
